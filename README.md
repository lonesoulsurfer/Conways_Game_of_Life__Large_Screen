...
- **Cyclic CA** — A multi-state automaton where cells cycle through states 0→1→2→…→N→0. Cells advance only when they have a neighbour in the next state. Produces stunning rotating spiral waves. Choose small, medium, or large seedings.
...
- **Lenia Sandbox** — A continuous cellular automaton with its own control scheme; see below.

## Lenia Sandbox

Unlike every automaton above, Lenia isn't discrete — cells hold a value from 0–255 instead of a simple on/off state, and each step blends that value with a weighted neighbourhood (the kernel) rather than applying binary birth/survival rules. The result is smooth, organic, blob-like life forms that grow, drift, and dissolve continuously.
...
| Up + Down | Toggle colour scheme (blue/cyan/white ↔ fire) |
...
| Up + Down (title screen) | Enter games menu — play any of the games or watch the cinematic shorts |
